...
struct SDPListTests {
    ...
    @Test func appendRemove() {
        var a = 1
        var b = 2
        let list1 = withUnsafeMutablePointer(to: &a) { BluetoothABI.sdp_list_append(nil, UnsafeMutableRawPointer($0)) }
        let list2 = withUnsafeMutablePointer(to: &b) { BluetoothABI.sdp_list_append(list1, UnsafeMutableRawPointer($0)) }
        #expect(list2 != nil)
        ...
        let removed = withUnsafeMutablePointer(to: &a) {
            BluetoothABI.sdp_list_remove(list2, UnsafeMutableRawPointer($0))
        }
        #expect(removed!.pointee.data!.load(as: Int.self) == 2)
        #expect(removed!.pointee.next == nil)
        BluetoothABI.sdp_list_free(removed, nil)
    }
}
...
struct SDPUUIDTests {
    ...
    @Test func create() {
        var u = uuid_t()
        BluetoothABI.sdp_uuid16_create(&u, 0x180D)
    ...
    @Test func toUUID128() {
        var u16 = uuid_t()
        BluetoothABI.sdp_uuid16_create(&u16, 0x180D)
        let u128 = BluetoothABI.sdp_uuid_to_uuid128(&u16)
    ...
    @Test func cmp() {
        var u1 = uuid_t()
        var u2 = uuid_t()
        BluetoothABI.sdp_uuid16_create(&u1, 0x180D)
        BluetoothABI.sdp_uuid16_create(&u2, 0x180D)
        #expect(BluetoothABI.sdp_uuid_cmp(&u1, &u2) == 0)
        BluetoothABI.sdp_uuid16_create(&u2, 0x180E)
        #expect(BluetoothABI.sdp_uuid_cmp(&u1, &u2) != 0)
    }

    @Test func uuid128ToUUID() {
        var u = uuid_t()
        BluetoothABI.sdp_uuid32_create(&u, 0x0000180D)
        let u128 = BluetoothABI.sdp_uuid_to_uuid128(&u)!
        let shortened = BluetoothABI.sdp_uuid128_to_uuid(u128)
        #expect(shortened == 1)
...
struct SDPDataTests {
    ...
    @Test func allocInt() {
        var value: UInt32 = 42
        let d = withUnsafePointer(to: &value) { BluetoothABI.sdp_data_alloc(UInt8(SDP_UINT32), $0) }
        #expect(d != nil)
        #expect(d!.pointee.dtd == UInt8(SDP_UINT32))
        #expect(d!.pointee.val.uint32 == 42)
        BluetoothABI.sdp_data_free(d)
    }

    @Test func allocString() {
        let d = "hello".withCString { BluetoothABI.sdp_data_alloc(UInt8(SDP_TEXT_STR8), $0) }
        #expect(d != nil)
    ...
    @Test func seqAppend() {
        var v1: UInt8 = 1
        var v2: UInt8 = 2
        let d1 = withUnsafePointer(to: &v1) { BluetoothABI.sdp_data_alloc(UInt8(SDP_UINT8), $0) }!
        let d2 = withUnsafePointer(to: &v2) { BluetoothABI.sdp_data_alloc(UInt8(SDP_UINT8), $0) }!
        let seq = BluetoothABI.sdp_seq_append(d1, d2)
        #expect(seq == d1)
        #expect(seq!.pointee.next == d2)
        BluetoothABI.sdp_data_free(seq)
    }
}
...
struct SDPRecordTests {
    ...
    @Test func allocFree() {
        let rec = BluetoothABI.sdp_record_alloc()
        #expect(rec != nil)
        #expect(rec!.pointee.handle == 0xffffffff)
        BluetoothABI.sdp_record_free(rec)
    }
    ...
    @Test func attrAddGetRemove() {
        ...
        var value: UInt32 = 7
        let d = withUnsafePointer(to: &value) { BluetoothABI.sdp_data_alloc(UInt8(SDP_UINT32), $0) }!
        #expect(BluetoothABI.sdp_attr_add(rec, 0x0002, d) == 0)

        let fetched = BluetoothABI.sdp_data_get(rec, 0x0002)
        #expect(fetched != nil)
        #expect(fetched!.pointee.val.uint32 == 7)

        BluetoothABI.sdp_attr_remove(rec, 0x0002)
        #expect(BluetoothABI.sdp_data_get(rec, 0x0002) == nil)
        ...
        "My Service".withCString { name in
            "My Provider".withCString { provider in
                "My Description".withCString { description in
                    BluetoothABI.sdp_set_info_attr(rec, name, provider, description)
                }
            }
        }

        var value = [CChar](repeating: 0, count: 64)
        #expect(BluetoothABI.sdp_get_string_attr(rec, UInt16(SDP_ATTR_SVCNAME_PRIMARY), &value, 64) == 0)
    ...
    @Test func serviceIdRoundTrip() {
        let rec = BluetoothABI.sdp_record_alloc()!
        var uuid = uuid_t()
        BluetoothABI.sdp_uuid16_create(&uuid, 0x1101)
        BluetoothABI.sdp_set_service_id(rec, uuid)

        var fetched = uuid_t()
        #expect(BluetoothABI.sdp_get_service_id(rec, &fetched) == 0)
    ...
    @Test func copyRecord() {
        ...
        let copy = BluetoothABI.sdp_copy_record(rec)!
        var fetched = uuid_t()
        #expect(BluetoothABI.sdp_get_service_id(copy, &fetched) == 0)
        #expect(fetched.value.uuid16 == 0x1101)

        BluetoothABI.sdp_record_free(rec)
        BluetoothABI.sdp_record_free(copy)
    }
}
...
struct SDPCodecTests {
    ...
    @Test func recordPDURoundTrip() throws {
        ...
        #expect(BluetoothABI.sdp_attr_add(rec, UInt16(SDP_ATTR_SVCLASS_ID_LIST), seq) == 0)

        var buf = sdp_buf_t()
        #expect(BluetoothABI.sdp_gen_record_pdu(rec, &buf) == 0)
        #expect(buf.data_size > 0)

        var scanned: Int32 = 0
        let decoded = buf.data!.withMemoryRebound(to: UInt8.self, capacity: Int(buf.data_size)) {
            BluetoothABI.sdp_extract_pdu($0, Int32(buf.data_size), &scanned)
        }
        #expect(decoded != nil)
        #expect(scanned == Int32(buf.data_size))

        let decodedList = BluetoothABI.sdp_data_get(decoded, UInt16(SDP_ATTR_SVCLASS_ID_LIST))
        #expect(decodedList != nil)
        #expect(decodedList!.pointee.val.dataseq!.pointee.val.uuid.value.uuid16 == 0x1101)

        free(buf.data)
        BluetoothABI.sdp_record_free(rec)
        BluetoothABI.sdp_record_free(decoded)
    }
    ...
    @Test func seq8UpgradesToSeq16OnOverflow() {
        ...
        var buf = sdp_buf_t()
        let size = BluetoothABI.sdpGenerateBuffer(&buf, seq)
        #expect(size > 0)
        buf.data = malloc(Int(buf.buf_size))?.assumingMemoryBound(to: UInt8.self)
        buf.data_size = 0
        _ = BluetoothABI.sdp_gen_pdu(&buf, seq)
