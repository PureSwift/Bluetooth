...
final class HCITests: XCTestCase {
    ...
    func testWriteLocalName() {
        ...
        do {
            ...
            let eventData: [UInt8] = [0x00, 0x0E, 0x04, 0x01, 0x13, 0x0C, 0x00]
            
            let testCommand = TestHostController.Command(command: commandData, events: [eventData])
            
            var hostController = TestHostController()
            hostController.commands[opcode] = testCommand
